...
auth.onAuthStateChanged(user => {
    ...
    if (user) {
        currentUser = user;
        checkIfBanned(user);
        isOwner = (user.email && user.email === ADMIN_EMAIL);
...
function toggleReaction(msgKey, emoji) {
    if (!currentUser) return;
    const ref = database.ref(`messages/${currentRoomId}/${msgKey}/reactions/${emoji}/${currentUser.uid}`);
    ref.once('value', snap => {
        if (snap.exists()) {
            ref.remove();
        } else {
...
// Profile photos now upload to Firebase Storage instead of being stored
// as giant base64 strings directly in the Realtime Database (which was
// bloating the DB fast — attachments already used Storage correctly,
// this brings profile photos in line with that).
...
function loadAdminUsers() {
    database.ref('users').once('value', snap => {
        ...
        allUsersCache.forEach(([uid, data]) => {
            ...
            const isTargetOwner = data.email === ADMIN_EMAIL;
            ...
            div.innerHTML = `
                ...
                <div>${moderationButtons}${ownerOnlyButtons}</div>
            `;
...
function isTargetTheOwner(uid) {
    ...
    return entry ? entry[1].email === ADMIN_EMAIL : false;
}
