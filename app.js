...
function loadAdminUsers() {
    database.ref('users').once('value', snap => {
        ...
        allUsersCache.forEach(([uid, data]) => {
            ...
            if (!isTargetOwner) {
                // Ban/mute/ghost/VIP/logout are available to the owner AND
                // to promoted moderators — but never usable against the
                // owner's own account, by anyone, including the owner's
                // account itself.
                moderationButtons = `
            ...
            div.innerHTML = `
                <span>${name} (${email}) ${isBanned} ${isTargetAdmin ? '🛡️' : ''} ${isTargetOwner ? '👑' : ''}</span>
                <div>${isTargetOwner ? '<span style="font-size:12px;color:#888;">This is you — the owner account, self-moderation disabled</span>' : ''}${moderationButtons}${ownerOnlyButtons}</div>
            `;
