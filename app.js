...
function displayMessage(msg) {
    ...
    // WhatsApp-style: long-press (or right-click on desktop) opens the
    // reaction + reply + delete action menu — no permanently visible
    // buttons cluttering every message.
    attachLongPress(div, (x, y) => openMessageActionMenu(msg, x, y));
...
// ============================================
// LONG-PRESS DETECTION (WhatsApp-style)
// Works with touch (mobile) and falls back to right-click on desktop.
// A short tap/click does nothing special; only a genuine hold (500ms,
// without significant finger movement) opens the action menu.
// ============================================
...
// Long-press a message → emoji row (react) + Reply + Delete (own msgs only).
...
function openMessageActionMenu(msg, x, y) {
    ...
    replyOpt.className = 'msg-action-item';
    replyOpt.textContent = '↩️ Reply';
    replyOpt.addEventListener('click', () => {
        startReply(msg);
        closeMessageActionMenu();
    });
    options.appendChild(replyOpt);
