...
// works with an unsigned upload preset, no card needed.
// ============================================
const CLOUDINARY_CLOUD_NAME = 'zaq3txco';
const CLOUDINARY_UPLOAD_PRESET = 'chitchat_preset';

// resourceType: 'image' for photos, 'auto' lets Cloudinary detect
// images/pdf/doc/txt automatically (used for chat attachments).
...
let currentUserRecord = {}; // live copy of users/{uid} for currentUser, kept fresh below — used to resolve MY OWN display name consistently (never a raw email/uid)
let currentRoomId = 'room1';
let isAdmin = false;
let isOwner = false;
let isModerator = false; // 3rd permission tier — day-to-day moderation only, no economy/system access
let isMuted = false;
let isGhost = false;
let isVIP = false;
...
let messagesChangedCallback = null;
let pendingPhotoURL = null;
let confirmedPhotoURL = null; // the last known-good uploaded URL (never a base64 preview)
...
let privateChatQuery = null;
let privateChatAddedCallback = null;
let privateChatChangedCallback = null;
...
// ============================================
// SHARED DISPLAY NAME RESOLVER
// ============================================
// Single source of truth for "what name do we show for this user" —
// used everywhere a user's name is rendered (All Users list, Admin
// Users tab, PM/Messenger, mentions, welcome messages, chat messages)
// so a user is NEVER shown as a raw email address or raw UID string.
// Priority: their own chosen profile name -> Guest# id -> a friendly
// name derived from their email's local-part -> 'User'.
// Exposed on window so other feature files (messenger/admin) can reuse
// this exact same logic instead of duplicating/diverging fallbacks.
function resolveDisplayName(uid, data) {
    data = data || {};
    if (data.profile && data.profile.displayName && data.profile.displayName.trim()) {
...
// ============================================
// PRESENCE-SAFE SIGN OUT
// ============================================
// Plain auth.signOut() does NOT close the Realtime Database connection
// (Auth and the DB socket are separate) — so onDisconnect() never fires
// on a manual logout, and the account is left showing "online" forever
// until the tab is actually closed. This writes offline first so the
// online count / All Users list are correct immediately after logout.
function signOutWithPresence() {
    if (currentUser) {
        return statusRef.child(currentUser.uid).set({ state: 'offline', lastSeen: Date.now() })
            .catch(() => {}).then(() => auth.signOut());
    }
    return auth.signOut();
}
...
signupForm.addEventListener('submit', e => {
    ...
    auth.createUserWithEmailAndPassword(email, pass)
        .then(cred => {
            ...
            alert('Account created! Please check your email to verify your account. Taking you to the chat...');
        })
...
logoutBtn.addEventListener('click', () => {
    signOutWithPresence().then(() => showAuth());
});
...
function showAuth() {
    ...
    if (messagesListenerRoomRef) {
        ...
        if (messagesChangedCallback) messagesListenerRoomRef.off('child_changed', messagesChangedCallback);
        messagesListenerRoomRef = null;
        messagesAddedCallback = null;
        messagesRemovedCallback = null;
        messagesChangedCallback = null;
    }
}
...
auth.onAuthStateChanged(user => {
    ...
    if (user) {
        currentUser = user;
        checkIfBanned(user);
        checkForceLogout(user);
        database.ref('users/' + user.uid).on('value', s => { currentUserRecord = s.val() || {}; });
        isOwner = !!(user.email && user.email.trim().toLowerCase() === ADMIN_EMAIL);
        isAdmin = isOwner; // moderator status (DB isAdmin flag) is layered on top by listenModerationFlags()
        if (isOwner) {
...
function saveUserRecord(user) {
    ...
    userRef.once('value', snap => {
        const isNewUser = !snap.exists() || !snap.val().createdAt;
        const updates = { uid: user.uid, lastSeen: Date.now() };
        if (user.email) updates.email = user.email;
        if (isNewUser) updates.createdAt = Date.now();
        userRef.update(updates).then(() => {
            if (isNewUser) {
                const displayName = resolveDisplayName(user.uid, Object.assign({}, snap.val(), updates));
                database.ref('messages/room1').push({
                    uid: user.uid,
                    name: 'ChitChat',
                    text: `👋 Welcome to ChitChat Pakistan, ${displayName}!`,
                    timestamp: Date.now(),
                    isSystem: true
                });
            }
...
// Best-effort IP capture using a free lookup API (client-side, no backend
// needed). This is what makes the admin panel's "📡 IP" button actually
// show something instead of "Not stored". If the lookup fails (offline,
// rate-limited, blocked) we just skip it silently — it's not critical path.
function captureUserIP(uid) {
    fetch('https://ipapi.co/json/')
        .then(res => res.ok ? res.json() : Promise.reject())
        .then(data => {
            if (data && data.ip) {
                database.ref('users/' + uid + '/ip').set(data.ip);
                if (data.country_name) database.ref('users/' + uid + '/ipCountry').set(data.country_name);
            }
        })
        .catch(() => { /* silent — IP capture is best-effort, never blocks login */ });
}
...
function listenModerationFlags(user) {
    ...
    // A promoted moderator (isAdmin flag set by the owner) gets admin panel
    // access too — but isOwner stays separate and controls the sensitive
    // owner-only actions (see loadAdminUsers / adminAction).
    database.ref('users/' + user.uid + '/isAdmin').on('value', snap => {
        isAdmin = isOwner || !!snap.val();
        document.body.classList.toggle('is-admin', isAdmin);
    });
    database.ref('users/' + user.uid + '/isModerator').on('value', snap => {
        isModerator = !!snap.val();
        document.body.classList.toggle('is-moderator', isModerator);
    });
}

function checkIfBanned(user) {
    // Real-time listener: if an admin bans this user while they are
    // already chatting, they get kicked out immediately instead of only
    // being caught the next time they log in.
    database.ref('banned_users/' + user.uid).on('value', snap => {
...
function checkForceLogout(user) {
    // Real-time listener: adminForceLogout() sets status/{uid}.forceLogout=true.
    // Previously nothing ever read this flag, so the admin button did nothing.
    database.ref('status/' + user.uid + '/forceLogout').on('value', snap => {
        if (snap.val() === true) {
            database.ref('status/' + user.uid + '/forceLogout').set(null);
...
let latestRoomsData = null;
let roomsListenerAttached = false;

function loadRooms() {
    if (!currentUser) return;
    database.ref('users/' + currentUser.uid + '/joinedRooms').once('value', snap => {
        joinedRoomsCache = snap.val() || {};
        if (!roomsListenerAttached) {
            attachRoomsListener();
            roomsListenerAttached = true;
        } else {
            renderRoomTabs(); // membership just (re)loaded — refresh with existing room data
        }
    });
}
...
function attachRoomsListener() {
    roomsRef.orderByChild('order').on('value', snap => {
        ...
        latestRoomsData = rooms;
        renderRoomTabs();
    });
}

// Rebuilds the room tab list from the last-known rooms data + membership
// cache. Safe to call any time (e.g. right after leaving a room) without
// re-subscribing to Firebase.
function renderRoomTabs() {
    const rooms = latestRoomsData;
    if (!rooms) return;
    ...
    roomIds.forEach(id => {
        ...
        const isJoined = id === 'room1' || !!joinedRoomsCache[id] || isAdmin;
        const tab = document.createElement('button');
        tab.className = 'room-tab' + (id === currentRoomId ? ' active' : '') + (isJoined ? '' : ' not-joined');
        tab.dataset.roomId = id;
        tab.innerHTML = `<span class="room-tab-label">${escapeHtml(room.name || id)}</span>`;
        tab.addEventListener('click', () => {
            if (isJoined) {
                joinRoom(id);
            } else {
                joinRoomFlow(id, room);
            }
        });
    ...
    attachRoomUnreadListeners(roomIds);
    populateRoomFilter();
}

// Explicit "Join" action for a room the user hasn't joined yet — writes
// membership, posts a WhatsApp/Discord-style welcome message naming the
// user, then switches into the room.
function joinRoomFlow(roomId, room) {
    if (!currentUser) return;
    if (!confirm(`Join "${room.name || roomId}"?`)) return;
    database.ref('users/' + currentUser.uid + '/joinedRooms/' + roomId).set(true).then(() => {
        joinedRoomsCache[roomId] = true;
        const displayName = resolveDisplayName(currentUser.uid, currentUserRecord);
...
// ============================================
let unreadCounts = {};
let roomUnreadListeners = {};

function attachRoomUnreadListeners(roomIds) {
    const sessionStart = Date.now();
    roomIds.forEach(id => {
        if (roomUnreadListeners[id]) return; // already listening
        const ref = database.ref('messages/' + id).orderByChild('timestamp').startAt(sessionStart);
        ...
        roomUnreadListeners[id] = { ref, cb };
    });
}

function updateRoomBadge(roomId) {
    const tab = roomTabsList.querySelector(`.room-tab[data-room-id="${roomId}"]`);
    if (!tab) return;
...
function attachMessagesListener(roomId) {
    ...
    // Reactions live on the message object itself, so a reaction toggle
    // fires child_changed — re-render just that message's reaction pills.
    messagesChangedCallback = roomQuery.on('child_changed', function(snapshot) {
        const msg = snapshot.val();
        if (!msg) return;
        const el = chatMessages.querySelector(`[data-id="${snapshot.key}"] .reactions`);
        if (el) renderReactions(el, msg.reactions);
    });
}
...
function timeStrFor(msg) {
    return msg.timestamp ? new Date(msg.timestamp).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'}) : 'Just now';
}
...
function displayMessage(msg) {
    ...
    // System / bot messages (e.g. "X joined the room") — plain centered text,
    // no avatar, no reactions, no reply, no delete.
    if (msg.isSystem) {
    ...
    let replyHTML = '';
    if (msg.replyTo) {
        replyHTML = `<div class="quoted-reply"><b>${escapeHtml(msg.replyTo.name)}</b>: ${escapeHtml(msg.replyTo.text)}</div>`;
    }

    // Content
    let content = escapeHtml(msg.text || '');
    let mediaOnlyHTML = '';
    const isImageOnlyMessage = !msg.isSticker && !msg.text && !msg.replyTo &&
        msg.attachments && msg.attachments.length > 0 &&
        msg.attachments.every(att => (att.type && att.type.startsWith('image/')) ||
            (!att.type && /\.(jpe?g|png|gif|webp|bmp|svg)$/i.test(att.name || '')));
    ...
    if (msg.isSticker && msg.stickerUrl) {
        ...
        // WhatsApp/Facebook-style: just the photo, no bubble background/padding
        // around it — the timestamp sits overlaid on the image itself.
        content = '';
        msg.attachments.forEach((att, i) => {
            ...
            mediaOnlyHTML += `<a href="${safeUrl}" target="_blank" rel="noopener noreferrer" class="media-only-img-wrap">
                <img src="${safeUrl}" alt="${escapeHtml(att.name)}" loading="lazy">
                ${isLast ? `<span class="media-time-overlay">${timeStrFor(msg)}</span>` : ''}
            </a>`;
        });
    } else {
        content = filterBadWords(content);
        if (currentUser) {
            const myName = resolveDisplayName(currentUser.uid, currentUserRecord);
            content = highlightMentions(content, myName);
        }
        if (msg.attachments && msg.attachments.length) {
            msg.attachments.forEach(att => {
                const safeUrl = (att.url || '').startsWith('http') ? escapeHtml(att.url) : '#';
                const looksLikeImage = (att.type && att.type.startsWith('image/')) ||
                    (!att.type && /\.(jpe?g|png|gif|webp|bmp|svg)$/i.test(att.name || ''));
                if (looksLikeImage && safeUrl !== '#') {
                    content += `<div class="attachment attachment-image"><a href="${safeUrl}" target="_blank" rel="noopener noreferrer"><img src="${safeUrl}" alt="${escapeHtml(att.name)}" loading="lazy"></a></div>`;
                } else {
    ...
    if (isImageOnlyMessage) div.classList.add('media-only');
    const timeStr = timeStrFor(msg);
    div.innerHTML = isImageOnlyMessage ? `
        <div style="display:flex; align-items:center; gap:8px; margin-bottom:4px;">
            ${avatarHTML}
            <span class="sender-name">${msg.isVIP ? '⭐ ' : ''}${safeName}</span>
        </div>
        <div class="media-only-grid">${mediaOnlyHTML}</div>
        <div class="reactions" data-msgkey="${msg.key}"></div>
    ` : `
        <div style="display:flex; align-items:center; gap:8px; margin-bottom:4px;">
            ${avatarHTML}
            <span class="sender-name">${msg.isVIP ? '⭐ ' : ''}${safeName}</span>
        ...
        <div class="reactions" data-msgkey="${msg.key}"></div>
    `;
    renderReactions(div.querySelector('.reactions'), msg.reactions);

    // WhatsApp-style: long-press (or right-click) opens the reaction +
    // delete menu; horizontal swipe triggers reply. No permanently
    // visible buttons cluttering every message.
    attachMessageGestures(div, msg);
...
function renderReactions(container, reactions) {
    ...
    Object.entries(reactions).forEach(([emoji, uids]) => {
        const count = uids ? Object.keys(uids).length : 0;
        if (count === 0) return;
        const mine = uid && uids && uids[uid];
...
function toggleReaction(msgKey, emoji) {
    ...
    const reactionsRef = database.ref(`messages/${currentRoomId}/${msgKey}/reactions`);
    reactionsRef.once('value', snap => {
        const reactions = snap.val() || {};
        const alreadyThisEmoji = !!(reactions[emoji] && reactions[emoji][uid]);
        const updates = {};
        // Only one reaction per user per message — remove any of their
        // OTHER reactions on this message before applying the new one.
        Object.keys(reactions).forEach(existingEmoji => {
            if (reactions[existingEmoji] && reactions[existingEmoji][uid]) {
                updates[`${existingEmoji}/${uid}`] = null;
            }
        });
        // Tapping the same reaction again removes it (un-react); tapping a
        // different one switches to it.
        if (!alreadyThisEmoji) {
            updates[`${emoji}/${uid}`] = true;
        }
...
// ============================================
// MESSAGE GESTURES — WhatsApp-style
// • Long-press (or right-click on desktop) → reaction row + Delete
//   (no Reply here anymore — matches WhatsApp/Messenger where the popup
//   menu is only for reactions/delete/etc, never reply).
// • Horizontal swipe (left OR right) → reply, with the bubble sliding
//   and a reply icon fading in, exactly like WhatsApp/Messenger's
//   swipe-to-reply gesture.
// ============================================
function attachMessageGestures(el, msg) {
    ...
    function onStart(x, y) {
        startX = x; startY = y; currentDX = 0;
        dragging = false;
        el.style.transition = 'none';
        timer = setTimeout(() => {
            if (!dragging) openMessageActionMenu(msg, x, y);
        }, 500);
    }
    ...
    function onMove(x, y) {
        ...
        if (!dragging) {
            if (Math.abs(dx) > 10 && Math.abs(dx) > Math.abs(dy)) {
                dragging = true;
                clearTimeout(timer); // a horizontal drag started — this isn't a long-press
            } else if (Math.abs(dy) > 10) {
                clearTimeout(timer); // vertical scroll — also not a long-press
            }
            return;
        }
        currentDX = Math.max(-SWIPE_MAX, Math.min(SWIPE_MAX, dx));
        el.style.transform = `translateX(${currentDX}px)`;
        replyIcon.style.opacity = String(Math.min(1, Math.abs(currentDX) / SWIPE_TRIGGER));
        replyIcon.style.left = currentDX > 0 ? '-28px' : '';
        replyIcon.style.right = currentDX < 0 ? '-28px' : '';
    }

    function onEnd() {
        clearTimeout(timer);
        if (dragging && Math.abs(currentDX) > SWIPE_TRIGGER) {
            startReply(msg);
        }
        resetDrag();
    }
    ...
    el.addEventListener('touchcancel', () => { clearTimeout(timer); resetDrag(); });

    // Desktop fallback — right-click opens the same menu instead of the
...
// Long-press a message → emoji row (react) + Delete (own msgs only).
// Reply is NOT here — it's the swipe gesture (see attachMessageGestures).
// Position is always clamped inside the viewport so it can never push the
// page into a horizontal scroll / blank area off-screen.
// ============================================
function openMessageActionMenu(msg, x, y) {
    closeMessageActionMenu();

    const backdrop = document.createElement('div');
    backdrop.className = 'msg-action-backdrop';
    backdrop.addEventListener('click', closeMessageActionMenu);
    backdrop.addEventListener('touchstart', closeMessageActionMenu);
    ...
    reactionRow.className = 'msg-action-reactions';
    REACTION_EMOJIS.forEach((emoji, i) => {
        const span = document.createElement('span');
        span.textContent = emoji;
        span.style.animationDelay = (i * 0.03) + 's';
        span.addEventListener('click', () => {
            toggleReaction(msg.key, emoji);
            closeMessageActionMenu();
        });
    ...
    if (isMine && !msg.isSticker) {
        ...
        options.className = 'msg-action-options';
        const delOpt = document.createElement('div');
        delOpt.className = 'msg-action-item delete';
        delOpt.textContent = '🗑️ Delete';
        delOpt.addEventListener('click', () => {
            deleteMessage(currentRoomId, msg.key);
            closeMessageActionMenu();
        });
        options.appendChild(delOpt);
        menu.appendChild(options);
    }

    document.body.appendChild(menu);

    // Clamp position inside the viewport (fixes the earlier bug where the
    // picker could land off-screen and cause a horizontal scroll to blank
    // space).
    const rect = menu.getBoundingClientRect();
    let left = x - rect.width / 2;
    let top = y - rect.height - 12;
    left = Math.max(8, Math.min(left, window.innerWidth - rect.width - 8));
    if (top < 8) top = y + 12; // not enough room above -> show below instead
...
function closeMessageActionMenu() {
    document.querySelectorAll('.msg-action-menu, .msg-action-backdrop').forEach(el => el.remove());
}
...
function loadMentionableUsers() {
    database.ref('users').once('value', snap => {
        ...
        mentionUsersCache = Object.entries(users).map(([uid, u]) => resolveDisplayName(uid, u));
    });
}
...
messageInput.addEventListener('input', () => {
    ...
    const matches = mentionUsersCache.filter(n => n.toLowerCase().startsWith(fragment)).slice(0, 5);
    if (!matches.length) {
        mentionBox.style.display = 'none';
        return;
    }
    ...
    matches.forEach(name => {
        ...
        item.addEventListener('mousedown', (e) => {
            e.preventDefault(); // don't lose input focus before we edit it
            const newBefore = beforeCursor.replace(/@(\w*)$/, '@' + name + ' ');
            messageInput.value = newBefore + val.slice(cursor);
            mentionBox.style.display = 'none';
            messageInput.focus();
        });
...
messageInput.addEventListener('blur', () => {
    // small delay so a mousedown on a suggestion still registers first
    setTimeout(() => { mentionBox.style.display = 'none'; }, 150);
});
...
function sendMessage(text, isSticker = false, stickerUrl = '', attachments = []) {
    ...
    const msgData = {
        uid: currentUser.uid,
        name: resolveDisplayName(currentUser.uid, currentUserRecord),
        text: text || '',
        timestamp: Date.now(),
        isSticker: isSticker,
...
function startReply(msg) {
    replyingTo = {
        ...
        text: msg.isSticker ? '[sticker]' : (msg.text || '').slice(0, 80)
    };
    const preview = document.getElementById('replyPreview');
    preview.innerHTML = `
        <span class="reply-text">↩️ Replying to <b>${escapeHtml(replyingTo.name)}</b>: ${escapeHtml(replyingTo.text)}</span>
        <span class="reply-close">✕</span>
    `;
    preview.style.display = 'flex';
    preview.querySelector('.reply-close').addEventListener('click', clearReplyPreview);
    messageInput.focus();
}
    ...
    input.onchange = function(e) {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            if (file.size > 15 * 1024 * 1024) {
                alert(file.name + ' is over 15MB and was skipped.');
                continue;
            }
            uploadPromises.push(
                uploadToCloudinary(file, 'auto').then(url => {
                    attachments.push({ name: file.name, url: url, type: file.type || '' });
                })
            );
        }
...
function loadProfile() {
    ...
    database.ref('users/' + uid + '/profile').once('value', snap => {
        const data = snap.val() || {};
        confirmedPhotoURL = data.photoURL || 'default-avatar.png';
        profilePhoto.src = confirmedPhotoURL;
...
saveProfileBtn.addEventListener('click', function() {
    if (!currentUser) return;
    if (photoUploadInProgress) return alert('Photo is still uploading, please wait a moment and try again.');
    const uid = currentUser.uid;
    const updates = {
        displayName: profileName.value.trim() || currentUser.displayName || 'User',
        motto: profileMotto.value.trim() || '',
        // Falls back to the last CONFIRMED (already-uploaded) URL, never to
        // profilePhoto.src directly — that can still hold a local base64
        // preview if Save is clicked before the Cloudinary upload finishes.
        photoURL: pendingPhotoURL || confirmedPhotoURL || 'default-avatar.png'
    };
    database.ref('users/' + uid + '/profile').update(updates).then(() => {
        ...
        confirmedPhotoURL = updates.photoURL;
        pendingPhotoURL = null;
        alert('Profile saved!');
...
// Profile photos upload to Cloudinary instead of being stored as giant
// base64 strings directly in the Realtime Database (which was bloating
// the DB fast) or Firebase Storage (needs the paid Blaze plan).
profilePhotoInput.addEventListener('change', function(e) {
    ...
    uploadToCloudinary(file, 'image').then(url => {
        ...
        profilePhoto.src = confirmedPhotoURL || 'default-avatar.png'; // revert the preview
    }).finally(() => {
        photoUploadInProgress = false;
        saveProfileBtn.disabled = false;
    });
...
function loadAdminPanel() {
    ...
    document.querySelectorAll('.admin-tab[data-owner-only]').forEach(tab => {
        tab.style.display = isOwner ? '' : 'none';
    });
    // Moderators only get the day-to-day tabs: Users, Messages, Bans.
    // Rooms/BadWords/Stickers/Ads/Settings/Logs need full Admin.
    document.querySelectorAll('.admin-tab').forEach(tab => {
        const tabName = tab.dataset.tab;
        const moderatorSafeTabs = ['users', 'messages', 'bans'];
        if (!isAdmin && !moderatorSafeTabs.includes(tabName)) {
    ...
    // Full database backup/export and "wipe everything" stay Owner-only —
    // these map to the agreed table's "DB Export/Delete All" row.
    adminExportAllDataBtn.style.display = isOwner ? '' : 'none';
    adminClearAllMessagesBtn.style.display = isOwner ? '' : 'none';
}
...
function loadAdminUsers() {
    database.ref('users').once('value', snap => {
        ...
        allUsersCache.forEach(([uid, data]) => {
            ...
            const name = escapeHtml(resolveDisplayName(uid, data));
            const isBanned = data.isBanned ? '🚫' : '';
            const isTargetOwner = (data.email || '').trim().toLowerCase() === ADMIN_EMAIL;
            const isTargetAdmin = !!data.isAdmin;

            let moderationButtons = '';
            if (!isTargetOwner) {
                // Mute + Ban are safe for a Moderator to use day-to-day.
                // Ban uses a capped-duration wrapper for moderators (max
                // 24h) — full/permanent bans stay Admin+ only.
                if (isAdmin) {
                    moderationButtons += `<button class="orange" onclick="adminMuteUser('${uid}')">🔇 Mute</button>`;
                    moderationButtons += `<button onclick="adminBanUser('${uid}', prompt('Reason:'), parseInt(prompt('Expiry mins:')))">🚫 Ban</button>`;
                } else if (isModerator) {
                    moderationButtons += `<button class="orange" onclick="adminMuteUser('${uid}')">🔇 Mute</button>`;
                    moderationButtons += `<button onclick="moderatorBanUser('${uid}')">🚫 Ban (max 24h)</button>`;
                }
                // Logout/Ghost/VIP/IP touch account-level state or privileges
                // beyond simple day-to-day moderation — Admin+ only.
                if (isAdmin) {
                    moderationButtons += `
            ...
            // Owner-only controls: promoting/demoting admins and deleting
            // accounts. A promoted moderator never sees these buttons at
            // all — this is what keeps a moderator from ever being able to
            // touch admin status (their own, someone else's, or the
            // owner's), no matter what.
            let ownerOnlyButtons = '';
            if (isOwner && !isTargetOwner) {
                ownerOnlyButtons += `<button onclick="adminAction('delete', '${uid}')">Delete</button>`;
                if (isTargetAdmin) {
                    ownerOnlyButtons += `<button class="orange" onclick="adminAction('removeadmin', '${uid}')">Remove Admin</button>`;
                } else {
                    ownerOnlyButtons += `<button class="green" onclick="adminAction('makeadmin', '${uid}')">Make Admin</button>`;
                }
            }

            // Owner AND Admin can promote/demote Moderators (but never
            // touch Admin/Owner status — that stays Owner-only above).
            let moderatorMgmtButtons = '';
            if ((isOwner || isAdmin) && !isTargetOwner && !isTargetAdmin) {
                const isTargetModerator = !!data.isModerator;
                moderatorMgmtButtons = isTargetModerator
                    ? `<button class="orange" onclick="adminAction('removemoderator', '${uid}')">Remove Moderator</button>`
                    : `<button class="green" onclick="adminAction('makemoderator', '${uid}')">Make Moderator</button>`;
            }

            div.innerHTML = `
                <span>${name} (${email}) ${isBanned} ${isTargetAdmin ? '🛡️' : ''} ${!!data.isModerator ? '👮' : ''} ${isTargetOwner ? '👑' : ''}</span>
                <div>${isTargetOwner ? '<span style="font-size:12px;color:#888;">This is you — the owner account, self-moderation disabled</span>' : ''}${moderationButtons}${moderatorMgmtButtons}${ownerOnlyButtons}</div>
            `;
            div.dataset.search = ((data.email || '') + ' ' + (data.profile?.displayName || '')).toLowerCase();
            list.appendChild(div);
        });
        filterAdminList('adminUserSearch', 'adminUserList');
    });
}
...
function loadAdminMessages() {
    database.ref('messages').once('value', snap => {
        ...
        allMsgs.slice(0, 100).forEach(msg => {
            ...
            `;
            div.dataset.search = ((msg.name || '') + ' ' + (msg.text || '')).toLowerCase();
            list.appendChild(div);
        });
        filterAdminList('adminMessageSearch', 'adminMessageList');
    });
}

// Shared filter helper for the admin search boxes — previously these two
// inputs existed in the HTML but nothing was ever bound to them, so typing
// in them did nothing at all.
function filterAdminList(inputId, listId) {
...
document.getElementById('adminUserSearch')?.addEventListener('input', () => filterAdminList('adminUserSearch', 'adminUserList'));
document.getElementById('adminMessageSearch')?.addEventListener('input', () => filterAdminList('adminMessageSearch', 'adminMessageList'));
...
// ============================================
// No one — not even another admin — can moderate the owner's account.
// This checks the cached user list (populated by the Users tab); the
// real enforcement is in database.rules.json, this is just a fast
// client-side guard so the attempt fails immediately with a clear message.
function isTargetTheOwner(uid) {
    const entry = allUsersCache.find(([id]) => id === uid);
    return entry ? (entry[1].email || '').trim().toLowerCase() === ADMIN_EMAIL : false;
}

function adminForceLogout(uid) {
    if (!isAdmin) return;
    if (isTargetTheOwner(uid)) return alert('You cannot moderate the owner account.');
...
// Moderators can ban, but only for a capped duration (max 24h) — full or
// permanent bans stay Admin+ only via adminBanUser below.
function moderatorBanUser(uid) {
    if (!isAdmin && !isModerator) return;
    if (isTargetTheOwner(uid)) return alert('You cannot moderate the owner account.');
    const reason = prompt('Reason for ban (optional):') || 'No reason';
    let minutes = parseInt(prompt('Ban duration in minutes (max 1440 = 24h):'));
    if (!minutes || minutes <= 0) minutes = 60; // sensible default if left blank/invalid
    if (minutes > 1440) minutes = 1440; // moderators can never exceed 24h, no matter what they type
    const expiry = Date.now() + minutes * 60000;
    bansRef.child(uid).set({
        reason: reason,
    ...
    alert('User banned for ' + minutes + ' minute(s).');
    loadAdminBans();
}
...
function adminExportAllData() {
    if (!isOwner) return alert('Owner only.');
    // Reading database.ref() (the root) directly was never actually
    // possible — the root has no ".read" rule of its own (only its
    // children do), so Firebase silently denied the whole request.
    // Fetching each top-level collection separately respects each
    // collection's own rule and actually works.
    const paths = ['users', 'rooms', 'messages', 'settings', 'stickers', 'badwords', 'banned_users', 'activity_logs'];
    Promise.all(paths.map(p => database.ref(p).once('value').then(snap => [p, snap.val()])))
        .then(results => {
            const data = {};
            results.forEach(([p, val]) => { data[p] = val; });
...
// Firebase rules only grant write/delete permission at each individual
// message's own path — NOT at the shared "messages" or "messages/{room}"
// parent level. A single database.ref('messages').remove() therefore
// always failed silently with permission-denied. Deleting each message
// one at a time (which IS permitted per the rules) actually works.
function deleteAllMessagesInRoom(roomId) {
    return database.ref('messages/' + roomId).once('value').then(snap => {
        const msgs = snap.val();
        if (!msgs) return;
        const deletes = Object.keys(msgs).map(key => database.ref('messages/' + roomId + '/' + key).remove());
...
function adminClearAllMessages() {
    ...
    database.ref('messages').once('value').then(snap => {
        const rooms = snap.val();
        if (!rooms) { alert('No messages to delete.'); return; }
        const roomDeletes = Object.keys(rooms).map(roomId => deleteAllMessagesInRoom(roomId));
        Promise.all(roomDeletes).then(() => {
...
window.adminAction = function(action, id) {
    ...
    switch(action) {
        ...
        case 'unban':
            ...
            break;
        case 'delete':
            if (!isOwner) return alert('Only the owner can delete accounts.');
            ...
            break;
        case 'makeadmin':
            if (!isOwner) return alert('Only the owner can grant admin.');
            database.ref('users/' + id + '/isAdmin').set(true);
            alert('User is now admin.');
            loadAdminUsers();
            break;
        case 'removeadmin':
            if (!isOwner) return alert('Only the owner can revoke admin.');
            database.ref('users/' + id + '/isAdmin').set(false);
            alert('Admin access removed.');
            loadAdminUsers();
            break;
        case 'makemoderator':
            if (!isOwner && !isAdmin) return alert('Owner or Admin only.');
            database.ref('users/' + id + '/isModerator').set(true);
            alert('User is now a moderator.');
            loadAdminUsers();
            break;
        case 'removemoderator':
            if (!isOwner && !isAdmin) return alert('Owner or Admin only.');
            database.ref('users/' + id + '/isModerator').set(false);
...
document.querySelectorAll('.admin-tab').forEach(tab => {
    tab.addEventListener('click', function() {
        if (this.dataset.ownerOnly && !isOwner) return; // hidden for moderators, and blocked even if triggered another way
...
adminDeleteAllMessagesBtn.addEventListener('click', function() {
    ...
    deleteAllMessagesInRoom(roomFilter).then(() => {
        alert('Messages cleared for this room!');
        loadAdminMessages();
    }).catch(err => alert('Error: ' + err.message));
});
...
adminBanUserBtn.addEventListener('click', function() {
    ...
    if (input.includes('@')) {
        ...
        const match = allUsersCache.find(([uid, data]) => data.email && data.email.trim().toLowerCase() === input.trim().toLowerCase());
        if (!match) {
...
function loadAllUsers() {
    database.ref('users').once('value', snap => {
        ...
        list.innerHTML = '';
        if (!users) return;
        const sorted = Object.entries(users).sort((a,b) => resolveDisplayName(a[0], a[1]) > resolveDisplayName(b[0], b[1]) ? 1 : -1);
        sorted.forEach(([uid, data]) => {
            const div = document.createElement('div');
            div.className = 'user-item';
            const rawName = resolveDisplayName(uid, data);
            const name = escapeHtml(rawName);
            const userStatusRef = database.ref('status/' + uid + '/state');
            userStatusRef.once('value', snap => {
                ...
                div.querySelector('.pm-user-btn').addEventListener('click', () => {
                    usersModal.style.display = 'none';
                    openPrivateChat(uid, rawName);
                });
            });
            list.appendChild(div);
        });
...
leaveRoomBtn.addEventListener('click', function() {
    if (currentRoomId === 'room1') {
        return alert('You cannot leave the default room.');
    }
    if (!confirm('Leave this room? You can re-join it later from the room list.')) return;
    const leavingId = currentRoomId;
    database.ref('users/' + currentUser.uid + '/joinedRooms/' + leavingId).remove().then(() => {
        delete joinedRoomsCache[leavingId];
        joinRoom('room1');
        renderRoomTabs(); // refresh tabs so this room shows "Join" again
    }).catch(err => alert('Error leaving room: ' + err.message));
});
...
document.querySelectorAll('.dropdown-item').forEach(item => {
    item.addEventListener('click', function(e) {
        ...
        switch(action) {
            case 'create-room': if(isOwner) createRoom(); break;
            case 'delete-room': if(isOwner) deleteRoom(); break;
            case 'rename-room': if(isOwner) renameRoom(); break;
            case 'room-settings': if(isOwner) roomSettings(); break;
            case 'view-profile': openProfile(); break;
            case 'change-password': openPassword(); break;
            case 'admin-panel': openAdmin(); break;
            case 'logout': signOutWithPresence(); break;
...
function openAdmin() { if (!isAdmin && !isModerator) return alert('Admin only!'); loadAdminPanel(); adminModal.style.display = 'flex'; }
function openStickers() { stickerModal.style.display = 'flex'; }
function openUsers() { loadAllUsers(); usersModal.style.display = 'flex'; }
function openSiteSettings() { if (!isOwner) return alert('Owner only!'); openAdmin(); document.querySelector('.admin-tab[data-tab="settings"]').click(); }

// ============================================
// LEGAL MODAL (Terms / Privacy / Rules)
// ============================================
document.querySelectorAll('.legal-link').forEach(link => {
    link.addEventListener('click', e => {
        e.preventDefault();
        const target = link.dataset.legal;
        document.getElementById('legalModal').style.display = 'flex';
        document.querySelectorAll('.legal-tab').forEach(t => t.classList.toggle('active', t.dataset.legal === target));
        document.querySelectorAll('.legal-panel').forEach(p => {
            p.style.display = p.dataset.legalPanel === target ? 'block' : 'none';
        });
    });
});
document.querySelectorAll('.legal-tab').forEach(tab => {
    tab.addEventListener('click', () => {
        ...
        document.querySelectorAll('.legal-tab').forEach(t => t.classList.toggle('active', t === tab));
        document.querySelectorAll('.legal-panel').forEach(p => {
            p.style.display = p.dataset.legalPanel === target ? 'block' : 'none';
        });
    });
