...
const CACHE_NAME = 'chitchat-shell-v4';
...
self.addEventListener('fetch', function (event) {
    ...
    event.respondWith(
        // BUGFIX: this was previously plain fetch(request), which respects
        // normal HTTP caching — so even though the STRATEGY here is meant
        // to be "network-first, always fresh", a same-origin JS/CSS file
        // the browser (or Vercel's CDN) had already cached with a normal
        // Cache-Control header could be served straight from that HTTP
        // cache without ever actually reaching the network. That silently
        // defeated the whole point of this handler: deploying a fixed
        // file could look like nothing changed, because the "network"
        // fetch below was quietly answered from an old cached copy. Adding
        // {cache:'reload'} forces this fetch to always bypass HTTP caching
        // and hit the network for a byte-fresh response, same as the
        // install-time pre-cache above already does.
