...
(function () {
    ...
    // ============================================
    // BUGFIX (this session): caller-side blank video + one-way audio.
    //
    // Root cause: #callLocalVideo/#callRemoteVideo have their `srcObject`
    // assigned while the element is still hidden (its ancestor
    // #callActiveScreen is display:none until beginActiveCallUI() runs),
    // and the code relied purely on the HTML `autoplay` attribute after
    // that. On some Android WebView/PWA shells, a <video> that was
    // invisible when srcObject was assigned never actually starts
    // decoding, and simply toggling the ancestor to display:flex later
    // does NOT auto-resume it — an explicit, fresh .play() call is
    // required. This is the exact same autoplay-policy issue already
    // fixed above for the ringtone (see unlockRingtoneOnFirstGesture).
    //
    // This explains why it's consistently the CALLER who gets nothing:
    // the callee's "Accept" tap is a fresh user gesture that lands right
    // when their media starts, so their browser usually allows it; the
    // caller's original "Call" tap is several seconds stale by the time
    // the offer/answer/ICE round-trip finishes, so the same protection
    // silently blocks it far more often — on both the local preview AND
    // the incoming remote track (which is also where the remote AUDIO
    // rides for audio-only calls, so it explains the one-way-audio
    // reports too, not just blank video).
    //
    // Fix: explicitly call .play() every time srcObject is assigned AND
    // again the moment the element actually becomes visible, and if a
    // browser still blocks it, queue it to retry on the very next tap
    // anywhere on the page (same trick as the ringtone unlock).
    // ============================================
    let mgrPendingPlayEls = [];
    function attemptPlay(el) {
        if (!el) return;
        const p = el.play();
        if (p && typeof p.catch === 'function') {
            p.catch(function (err) {
                if (mgrPendingPlayEls.indexOf(el) === -1) mgrPendingPlayEls.push(el);
                console.warn('Messenger V5-A: call media play() blocked, will retry on next tap', err);
            });
        }
    }
    function retryPendingPlay() {
        if (!mgrPendingPlayEls.length) return;
        const els = mgrPendingPlayEls;
        mgrPendingPlayEls = [];
        els.forEach(attemptPlay);
    }
    document.addEventListener('click', retryPendingPlay, true);
    document.addEventListener('touchstart', retryPendingPlay, true);
    ...
    function createPeerConnection() {
        ...
        pc.ontrack = function (e) {
            ...
            // BUGFIX: don't rely on the `autoplay` attribute alone — see the
            // note above attemptPlay(). This is what actually carries the
            // remote person's AUDIO too (for audio-only calls), so this one
            // call fixes both the blank-video and one-way-audio reports.
            attemptPlay(remoteVideo);
        };
    ...
    function setupLocalPreview() {
        ...
        if (localVideo) {
            localVideo.srcObject = mgrLocalStream;
    ...
    function beginActiveCallUI() {
        ...
        // BUGFIX: callRemoteVideo used to get display:none for audio-only
        // calls. That's where the remote person's AUDIO track plays from —
        // on some devices, setting display:none on a <video> pauses/never-
        // starts its media entirely (audio included), which matches the
        // one-way-audio reports. It doesn't need hiding anyway: the avatar
        // below already renders on top of it (z-index), so leave it visible
        // and simply let the avatar cover it for audio calls.
        ...
        // BUGFIX: re-attempt playback now that the elements are actually
        // visible on screen. A .play() issued earlier while an ancestor was
        // still display:none can fail silently on some devices and does not
        // auto-resume just because display later changes — this is the
        // second (and most important) of the two retry points, see the
        // note above attemptPlay().
        attemptPlay(document.getElementById('callLocalVideo'));
        attemptPlay(document.getElementById('callRemoteVideo'));
