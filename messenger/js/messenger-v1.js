...
(function () {
    ...
    function wireMessengerUI() {
        ...
        document.getElementById('messengerVoiceSendBtn').addEventListener('click', function () {
        ...
        document.getElementById('messengerMsgInput').addEventListener('keydown', function (e) {
            if (e.key === 'Enter' && this.value.trim()) { e.preventDefault(); sendMessage(); }
        });
        document.getElementById('messengerMsgInput').addEventListener('input', updateVoiceSendIcon);
    }

    function updateVoiceSendIcon() {
        const input = document.getElementById('messengerMsgInput');
        const icon = document.getElementById('messengerVoiceSendIcon');
    ...
    // Chats store a one-time-captured `withName` snapshot from when the
    // chat started. If that name was ever wrong (raw email/uid captured
    // before the display-name fix, or the other person has since changed
    // their profile name), we resolve their CURRENT real name here and
    // patch it in wherever a contact name is shown — no DB migration
    // needed. Cached per uid so repeated renders don't re-fetch.
    const mgrNameCache = {};
    function mgrResolveLiveName(uid, fallback, cb) {
        if (!uid) { cb(fallback || 'User'); return; }
        if (mgrNameCache[uid]) { cb(mgrNameCache[uid]); return; }
        database.ref('users/' + uid).once('value', function (snap) {
            const resolved = (window.resolveDisplayName && window.resolveDisplayName(uid, snap.val())) || fallback || 'User';
            mgrNameCache[uid] = resolved;
            cb(resolved);
        }).catch(function () { cb(fallback || 'User'); });
    }
    ...
    function openChat(chatId, otherUid, otherName, otherPhoto) {
        ...
        mgrResolveLiveName(otherUid, otherName, function (name) {
            document.getElementById('messengerChatName').textContent = name;
        });
        showChatScreen();
    ...
    function renderInbox(snapshot) {
        ...
        // Badge count on the main nav tab must work even before the person
        // has ever opened Messenger (messenger.html, and so #messengerInboxList,
        // isn't injected into the page yet at that point) — compute and apply
        // it unconditionally, independent of whether the inbox list DOM exists.
        let totalUnread = 0;
        if (data) {
            Object.values(data).forEach(function (c) { totalUnread += (c.unreadCount || 0); });
        }
        updateMenuBadge(totalUnread);

        if (!listEl) return; // Messenger screen not open yet — badge above is all that's needed right now
        if (!data) {
            listEl.innerHTML = '';
        ...
        const chats = Object.entries(data).sort(function (a, b) {
            const pinDiff = (b[1].pinned ? 1 : 0) - (a[1].pinned ? 1 : 0);
        ...
        chats.forEach(function (entry) {
            ...
            const c = entry[1];
            if (!c.withUid) return;
            mgrResolveLiveName(c.withUid, c.withName, function (name) {
                const item = listEl.querySelector('.messenger-inbox-item[data-chat-id="' + CSS.escape(chatId) + '"] .messenger-inbox-name');
    ...
    // ============================================
    // V8: block sharing links / phone numbers in Messenger text
    // ============================================
    const MGR_URL_PATTERN = /(https?:\/\/|www\.|wa\.me\/|t\.me\/|\.com\b|\.pk\b|\.net\b|\.org\b|\.io\b)/i;
    const MGR_PHONE_PATTERN = /(\+?\d[\d\-\s]{7,}\d)/; // 9+ digits allowing spaces/dashes (covers 03XX-XXXXXXX style numbers)

    function mgrFlagAndBlock(chatId, text, matchType) {
        alert('Sharing links or phone numbers in Messenger is not allowed.');
        if (!currentUser) return;
        database.ref('moderationFlags').push({
            uid: currentUser.uid,
            chatId: chatId,
            text: text,
            matchType: matchType,
            timestamp: Date.now()
        }).catch(function (err) { console.error('Messenger: could not log moderation flag', err); });
    }
    ...
    function sendMessage() {
        ...
        if (MGR_URL_PATTERN.test(text) || MGR_PHONE_PATTERN.test(text)) {
            mgrFlagAndBlock(mgrOpenChatId, text, MGR_URL_PATTERN.test(text) ? 'link' : 'phone');
            return;
        }
        input.value = '';
        updateVoiceSendIcon();
    ...
    function startChat(otherUid, otherName, otherPhoto) {
        ...
        ensureHtmlInjected().then(function () {
            database.ref('users/' + myUid).once('value', function (mySnap) {
                const myData = mySnap.val() || {};
                const myName = (window.resolveDisplayName && window.resolveDisplayName(myUid, myData)) || currentUser.displayName || 'User';
        ...
        database.ref('users').once('value', function (snap) {
            ...
            mgrUsersCache = Object.entries(users)
                ...
                .map(function (entry) {
                    ...
                    return {
                        uid: uid,
                        name: (window.resolveDisplayName && window.resolveDisplayName(uid, d)) || 'User',
    ...
    window.messengerResolveLiveName = mgrResolveLiveName;
    ...
    auth.onAuthStateChanged(function (user) {
        if (user) {
            listenInbox(); // badge must be live from login, not just after first opening Messenger
        } else {
            detachInboxListener();
            hideMessengerUI();
        }
