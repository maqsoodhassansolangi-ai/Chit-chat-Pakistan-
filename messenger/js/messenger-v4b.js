...
(function () {
    ...
    function decorateBubbleWhenReady(key, attempt) {
        ...
        if (mediaEl && showDownload) {
            const dlBtn = document.createElement('button');
            dlBtn.className = 'messenger-media-download-btn';
            dlBtn.title = 'Save to device';
            dlBtn.textContent = '📥';
            dlBtn.addEventListener('click', function (e) {
                e.stopPropagation();
                downloadMedia(msg.url, msg.type);
            });
            el.insertBefore(dlBtn, mediaEl.nextSibling);
        }
    }

    // Saves media to the person's OWN device (not our server) — a plain
    // <a download> is silently ignored by most browsers for cross-origin
    // URLs like Cloudinary's, so we fetch the file as a blob first and
    // download that instead, which works reliably cross-origin.
    function downloadMedia(url, type) {
        fetch(url).then(function (res) { return res.blob(); }).then(function (blob) {
            ...
            a.href = objectUrl;
            a.download = 'chitchat-' + type + '-' + Date.now() + '.' + ext;
            document.body.appendChild(a);
            a.click();
            a.remove();
            setTimeout(function () { URL.revokeObjectURL(objectUrl); }, 4000);
        }).catch(function () {
            window.open(url, '_blank'); // fallback — at least let them save it manually
        });
    }
